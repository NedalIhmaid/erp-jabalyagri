...
use App\Policies\DailyVisitPolicy;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Tests\TestCase;

class DailyVisitPolicyTest extends TestCase
{
    use RefreshDatabase;

    protected DailyVisitPolicy $policy;

    protected function setUp(): void
    {
        parent::setUp();
        $this->policy = new DailyVisitPolicy();
    }
    ...
    {
        $user = User::factory()->create();
        $visit = DailyVisit::factory()->create(['user_id' => $user->id]);
        $this->assertIsBool($this->policy->view($user, $visit));
    }
    ...
    public function test_update_policy_exists(): void
    {
        $user = User::factory()->create();
        $visit = DailyVisit::factory()->create(['user_id' => $user->id]);
        $this->assertIsBool($this->policy->update($user, $visit));
    }
    ...
    public function test_delete_policy_exists(): void
    {
        $user = User::factory()->create();
        $visit = DailyVisit::factory()->create(['user_id' => $user->id]);
        $this->assertIsBool($this->policy->delete($user, $visit));
    }
    ...
    public function test_restore_policy_exists(): void
    {
        $user = User::factory()->create();
        $visit = DailyVisit::factory()->create(['user_id' => $user->id]);
        $this->assertIsBool($this->policy->restore($user, $visit));
    }
    ...
    public function test_force_delete_policy_exists(): void
    {
        $user = User::factory()->create();
        $visit = DailyVisit::factory()->create(['user_id' => $user->id]);
        $this->assertIsBool($this->policy->forceDelete($user, $visit));
    }
    ...
    public function test_replicate_policy_exists(): void
    {
        $user = User::factory()->create();
        $visit = DailyVisit::factory()->create(['user_id' => $user->id]);
        $this->assertIsBool($this->policy->replicate($user, $visit));
    }
